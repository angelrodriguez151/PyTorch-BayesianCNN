import torch.nn as nn
from layers.misc import FlattenLayer


def conv_init(m):
    classname = m.__class__.__name__
    if classname.find('Conv') != -1:
        #nn.init.xavier_uniform(m.weight, gain=np.sqrt(2))
        nn.init.normal_(m.weight, mean=0, std=1)
        nn.init.constant(m.bias, 0)

class ThreeConvThreeFC(nn.Module):
    """
    To train on CIFAR-10:
    https://arxiv.org/pdf/1207.0580.pdf
    """
    def __init__(self, outputs, inputs):
        super(ThreeConvThreeFC, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(inputs, 32, 6, stride=1, padding=2),
            nn.Softplus(),
            nn.MaxPool2d(kernel_size=8, stride=2),
            nn.Conv2d(32, 64, 6, stride=1, padding=2),
            nn.Softplus(),
            nn.MaxPool2d(kernel_size=8, stride=2),
            nn.Conv2d(64, 128, 6, stride=1, padding=1),
            nn.Softplus(),
            nn.MaxPool2d(kernel_size=8, stride=2), 
            nn.Conv2d(128, 128, 6, stride=1, padding=1),
            nn.Softplus(),
            nn.MaxPool2d(kernel_size=8, stride=2),
        )
        self.classifier = nn.Sequential(
            nn.Flatten(1,3),
            nn.Linear(8 * 8 * 128, 1000),
            nn.Softplus(),
            nn.Linear(1000, 1000),
            nn.Softplus(),
            nn.Linear(1000, outputs)
        )

    def forward(self, x):
        x = self.features(x)
        print(x.shape)
        x = self.classifier(x)
        return x
